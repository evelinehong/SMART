...
from processors.utils_ner import CNerTokenizer, get_entities, get_probs
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(test_dataloader):
        model.eval()
        batch = tuple(t.to(args.device) for t in batch)
        original_labels = batch[3].squeeze(0).cpu().numpy().tolist()
        with torch.no_grad():
        ...
        entities_with_probs = get_probs(preds, args.id2label, logits[0][1:-1])
        original_with_probs = get_probs(original_labels[1:-1], args.id2label, logits[0][1:-1])
        json_d = {}
        json_d['id'] = step
        json_d['tag_seq'] = " ".join([args.id2label[x] for x in preds])
        json_d['entities'] = label_entities
        json_d['probs'] = entities_with_probs
        json_d['orig'] = original_with_probs
    ...
    # if args.task_name == 'cluener' or args.task_name == 'math23k':
    ...
    for x, y in zip(test_text, results):
        ...
        json_d['orig_label'] = {}
        entities = y['entities']
        entities_probs = y['probs']
        original_probs = y['orig']
        words = list(x['text'])
        if len(entities) != 0:
            for subject in entities_probs:
                tag = subject[0]
                start = subject[1]
                end = subject[2]
                prob = subject[3]
                ...
                if tag in json_d['label']:
                    if word in json_d['label'][tag]:
                        ...
                        json_d2['prob'][tag][word] += prob
                        json_d['final_prob'][tag][word] = json_d2['prob'][tag][word] / len(json_d['label'][tag][word])
                    else:
                        json_d['label'][tag][word] = [[start, end]]
                        json_d2['prob'][tag][word] = prob
                        json_d['final_prob'][tag][word] = prob
                else:
                    json_d['label'][tag] = {}
                    json_d2['prob'][tag] = {}
                    json_d['final_prob'][tag] = {}
                    json_d['label'][tag][word] = [[start, end]]
                    json_d2['prob'][tag][word] = prob
                    json_d['final_prob'][tag][word] = prob

            for subject in original_probs:
                tag = subject[0]
                start = subject[1]
                end = subject[2]
                prob = subject[3]
                ...
                if tag in json_d['orig_label']:
                    if word in json_d['orig_label'][tag]:
                        json_d['orig_label'][tag][word].append([start, end])
                        json_d2['prob'][tag][word] += prob
                        json_d['orig_prob'][tag][word] = json_d2['prob'][tag][word] / len(json_d['orig_label'][tag][word])
                    else:
                        json_d['orig_label'][tag][word] = [[start, end]]
                        json_d2['prob'][tag][word] = prob
                        json_d['orig_prob'][tag][word] = prob
                else:
                    json_d['orig_label'][tag] = {}
                    json_d2['prob'][tag] = {}
                    json_d['orig_prob'][tag] = {}
                    json_d['orig_label'][tag][word] = [[start, end]]
                    json_d2['prob'][tag][word] = prob
                    json_d['orig_prob'][tag][word] = prob
